...
# Variables
####################################
LOG_FOLDER="Chemin/vers/le/dossier/logs/bowtie2"
RESULT_FOLDER="Chemin/vers/le/dossier/bowtie2_alignment_$(date +%Y%m%d_%H%M%S)"
FILENAME="Chemin/vers/le/dossier/All_RNA.fasta"
READS_FOLDER="Chemin/vers/le/dossier/fastp_20250527_124206"
...
bowtie2 \
  ...
  $BOWTIE2_OPTIONS \
  -x "$INDEX_PREFIX" \
  -1 "$READS_FOLDER/*_R1.fastq.gz" \
  -2 "$READS_FOLDER/*_R2.fastq.gz" \
  -S "$RESULT_FOLDER/output_align.sam" >> "$LOGFILE" 2>&1
