...
# Chemins (à adapter)
FILENAME="/home1/datawork/ltrouill/Ifremer/Data/Cleaned_data/Karlodinium/Long_reads/fastplong_20250516_074145/Karlodinium_cleaned.fastq"
...
if $RUN_STEP_2; then
    if ls "$EX_CLUSTERS_FOLDER"/*.fq 1> /dev/null 2>&1; then
        ...
        rattle extract_clusters -i "$FILENAME" -c "$CLUSTERED_TRANSCRIPTS" -o "$EX_CLUSTERS_FOLDER" --fastq >> "$LOG_FILE" 2>&1
...
if [ -f "$CORRECTED_READS" ]; then
    ...
    rattle correct -i "$FILENAME" -c "$CLUSTERED_TRANSCRIPTS" -t "$THREADS" $VERBOSE -o "$CORRECTED_FOLDER" -g 0.2 -m 0.4 -s 150 -r 10 >> "$LOG_FILE" 2>&1
...
if [ -f "$POLISHED_READS" ]; then
    ...
    rattle polish -i "$CONSENSI_FILE" -t "$THREADS" $VERBOSE -o "$RESULT_FOLDER" --summary >> "$LOG_FILE" 2>&1
